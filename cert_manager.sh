...
list_servers() {
    ...
    if [ -n "$TYPES" ]; then
        ...
        for T in $TYPES; do
            ...
            grep -v '^#' "$FILE" | grep -v '^$' | while IFS=: read -r H P TP; do
                TP=$(echo "$TP" | xargs)
                [ "$TP" = "$T" ] && echo -e "    ${DIM}•${RESET} ${H}:${P}"
            done
            echo ""
        done
    fi
...
delete_server() {
    ...
    while IFS= read -r LINE; do
        ...
        i=$((i + 1))
        LINES+=("$LINE")
        IFS=: read -r H P T <<< "$LINE"
        T=$(echo "$T" | xargs)
        echo -e "  ${GREEN}${i})${RESET} ${H}:${P} ${DIM}[${T}]${RESET}"
    done < "$FILE"
    ...
    if [[ "$NUM" =~ ^[0-9]+$ ]] && [ "$NUM" -ge 1 ] && [ "$NUM" -le $i ]; then
        ...
        IFS=: read -r H P T <<< "$TARGET"

        echo -ne "  ${RED}Remove ${H}:${P}?${RESET} (y/N): "
        read -r CONFIRM
